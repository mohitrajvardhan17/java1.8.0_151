  ...
  extends JPEGMetadataFormatResources
{
  ...
  protected Object[][] getContents()
  {
    ...
    int i = 0;
    int j = 0;
    while (j < commonContents.length)
    {
      arrayOfObject[i][0] = commonContents[j][0];
      arrayOfObject[i][1] = commonContents[j][1];
      j++;
      i++;
    }
    j = 0;
    while (j < imageContents.length)
    {
      arrayOfObject[i][0] = imageContents[j][0];
      arrayOfObject[i][1] = imageContents[j][1];
      j++;
      i++;
    }
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\imageio\plugins\jpeg\JPEGImageMetadataFormatResources.class
