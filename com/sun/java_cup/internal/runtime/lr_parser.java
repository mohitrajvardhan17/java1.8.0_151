...
public abstract class lr_parser
{
  ...
  {
    debug_message("# Reduce with prod #" + paramInt1 + " [NT=" + paramInt2 + ", SZ=" + paramInt3 + "]");
  }
    ...
    throws Exception
  {
    ...
    while (!_done_parsing)
    {
      ...
      else if (i < 0)
      {
        ...
        i = get_reduce(stack.peek()).parse_state, k);
        debug_message("# Reduce rule: top state " + stack.peek()).parse_state + ", lhs sym " + k + " -> state " + i);
        parse_state = i;
        used_by_parser = true;
        stack.push(localSymbol);
        tos += 1;
    ...
    throws Exception
  {
    ...
    for (;;)
    {
      ...
      if (paramBoolean) {
        debug_message("# Consuming Symbol #" + cur_err_tokensym);
      }
      restart_lookahead();
    }
    if (paramBoolean) {
    ...
    throws Exception
  {
    ...
    for (;;)
    {
      ...
      if (i > 0)
      {
        ...
        if (paramBoolean) {
          debug_message("# Parse-ahead shifts Symbol #" + cur_err_tokensym + " into state #" + (i - 1));
        }
      ...
      else
      {
        ...
        if (paramBoolean) {
          debug_message("# Parse-ahead reduces: handle size = " + k + " lhs = #" + j + " from state #" + localvirtual_parse_stack.top());
        }
    ...
    throws Exception
  {
    ...
    while (!_done_parsing)
    {
      int i = get_action(stack.peek()).parse_state, cur_err_tokensym);
      if (i > 0)
      {
      ...
      {
        report_fatal_error("Syntax error", localSymbol);
        return;
      }
    }
  }
  ...
  protected static short[][] unpackFromStrings(String[] paramArrayOfString)
  {
    ...
    i = 0;
    int j = localStringBuffer.charAt(i) << '\020' | localStringBuffer.charAt(i + 1);
    i += 2;
    short[][] arrayOfShort = new short[j][];
    for (int k = 0; k < j; k++)
    {
      int m = localStringBuffer.charAt(i) << '\020' | localStringBuffer.charAt(i + 1);
      i += 2;
      arrayOfShort[k] = new short[m];
      for (int n = 0; n < m; n++) {
        arrayOfShort[k][n] = ((short)(localStringBuffer.charAt(i++) - '\002'));
      }
    }
    return arrayOfShort;
  }
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\java_cup\internal\runtime\lr_parser.class
