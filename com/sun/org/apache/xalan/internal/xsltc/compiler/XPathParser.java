  ...
  extends lr_parser
{
  ...
  public XPathParser(Scanner paramScanner)
  {
    super(paramScanner);
  }
  ...
  public Symbol do_action(int paramInt1, lr_parser paramlr_parser, Stack paramStack, int paramInt2)
    throws Exception
  {
    return action_obj.CUP$XPathParser$do_action(paramInt1, paramlr_parser, paramStack, paramInt2);
  }
  ...
  {
    _parser = paramParser;
    _xsltc = paramParser.getXSLTC();
    _symbolTable = paramParser.getSymbolTable();
  }
  ...
  public StepPattern createStepPattern(int paramInt, Object paramObject, Vector paramVector)
  {
    ...
    QName localQName1 = (QName)paramObject;
    int j = 0;
    if (paramInt == 9)
    {
      i = localQName1.toString().equals("*") ? -1 : _xsltc.registerNamespacePrefix(localQName1);
    }
    else
    {
      localObject = localQName1.getNamespace();
      String str = localQName1.getLocalPart();
      QName localQName2 = _parser.getQNameIgnoreDefaultNs("namespace-uri");
      if ((localObject != null) && ((str.equals("*")) || (str.equals("@*"))))
      {
        if (paramVector == null) {
          paramVector = new Vector(2);
        }
        j = paramVector.size() == 0 ? 1 : 0;
        paramVector.add(new Predicate(new EqualityExpr(0, new NamespaceUriCall(localQName2), new LiteralExpr((String)localObject))));
      }
      if (str.equals("*")) {
        i = paramInt == 2 ? 2 : 1;
      } else if (str.equals("@*")) {
        ...
        i = paramInt == 2 ? _xsltc.registerAttribute(localQName1) : _xsltc.registerElement(localQName1);
      }
    }
    Object localObject = new StepPattern(paramInt, i, paramVector);
    if (j != 0) {
      ((StepPattern)localObject).setPriority(-0.25D);
    }
  ...
  {
    if (paramObject == null) {
      return paramInt == 9 ? -1 : paramInt == 2 ? 2 : 1;
    }
    if ((paramObject instanceof Integer)) {
    ...
    if (paramInt == 9) {
      return localQName.toString().equals("*") ? -1 : _xsltc.registerNamespacePrefix(localQName);
    }
    if (localQName.getNamespace() == null)
    {
      String str = localQName.getLocalPart();
      if (str.equals("*")) {
    ...
    return paramInt == 2 ? _xsltc.registerAttribute(localQName) : _xsltc.registerElement(localQName);
  }
  
  public Symbol parse(String paramString, int paramInt)
    throws Exception
  {
    try
    {
      _expression = paramString;
      _lineNumber = paramInt;
      return super.parse();
    }
    catch (IllegalCharException localIllegalCharException)
    {
      ErrorMsg localErrorMsg = new ErrorMsg("ILLEGAL_CHAR_ERR", paramInt, localIllegalCharException.getMessage());
      _parser.reportError(2, localErrorMsg);
    }
    return null;
  }
  ...
  {
    VariableBase localVariableBase = _parser.lookupVariable(paramQName);
    if (localVariableBase != null) {
      return localVariableBase;
    }
    return _symbolTable.lookupName(paramQName);
  }
  
  public final void addError(ErrorMsg paramErrorMsg)
  {
    _parser.reportError(3, paramErrorMsg);
  }
  
  public void report_error(String paramString, Object paramObject)
  {
    ErrorMsg localErrorMsg = new ErrorMsg("SYNTAX_ERR", _lineNumber, _expression);
    _parser.reportError(2, localErrorMsg);
  }
  ...
  public RelativeLocationPath insertStep(Step paramStep, RelativeLocationPath paramRelativeLocationPath)
  {
    ...
    {
      ParentLocationPath localParentLocationPath = (ParentLocationPath)paramRelativeLocationPath;
      RelativeLocationPath localRelativeLocationPath = insertStep(paramStep, localParentLocationPath.getPath());
      return new ParentLocationPath(localRelativeLocationPath, localParentLocationPath.getStep());
    }
    addError(new ErrorMsg("INTERNAL_ERR", "XPathParser.insertStep"));
    return paramRelativeLocationPath;
  }
  
  public boolean isElementAxis(int paramInt)
  {
    return (paramInt == 3) || (paramInt == 2) || (paramInt == 9) || (paramInt == 4);
  }
}


/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\org\apache\xalan\internal\xsltc\compiler\XPathParser.class
