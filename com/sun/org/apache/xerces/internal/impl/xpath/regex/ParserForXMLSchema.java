  ...
  extends RegexParser
{
      ...
      if (i1 == 0)
      {
        ...
        if ((read() != 0) || (chardata != 45) || ((n == 45) && (m != 0)))
        {
          if ((!isSet(2)) || (n > 65535)) {
            ...
            addCaseInsensitiveChar(localRangeToken2, n);
          }
        }
        else
        {
          ...
          else
          {
            ...
            if ((!isSet(2)) || ((n > 65535) && (i3 > 65535))) {
              ...
              addCaseInsensitiveCharRange(localRangeToken2, n, i3);
            }
          }
        }
      }
  ...
  protected static synchronized RangeToken getRange(String paramString, boolean paramBoolean)
  {
    if (ranges == null)
    {
      ...
      localRangeToken = Token.createRange();
      setupRange(localRangeToken, "09٠٩۰۹०९০৯੦੯૦૯୦୯௧௯౦౯೦೯൦൯๐๙໐໙༠༩၀၉፩፱០៩᠐᠙０９");
      setupRange(localRangeToken, DIGITS_INT);
      ranges.put("xml:isDigit", localRangeToken);
      ranges2.put("xml:isDigit", Token.complementRanges(localRangeToken));
      localRangeToken = Token.createRange();
      setupRange(localRangeToken, "AZazÀÖØöøıĴľŁňŊžƀǰǴǵǺȗɐʨʻˁʰˑΆΆΈΊΌΌΎΡΣώϐϖϚϚϜϜϞϞϠϠϢϳЁЌЎяёќўҁҐӄӇӈӋӌӐӫӮӵӸӹԱՖՙՙաֆאתװײءغفيٱڷںھۀێېۓەەۥۦअहऽऽक़ॡঅঌএঐওনপরললশহড়ঢ়য়ৡৰৱਅਊਏਐਓਨਪਰਲਲ਼ਵਸ਼ਸਹਖ਼ੜਫ਼ਫ਼ੲੴઅઋઍઍએઑઓનપરલળવહઽઽૠૠଅଌଏଐଓନପରଲଳଶହଽଽଡ଼ଢ଼ୟୡஅஊஎஐஒகஙசஜஜஞடணதநபமவஷஹఅఌఎఐఒనపళవహౠౡಅಌಎಐಒನಪಳವಹೞೞೠೡഅഌഎഐഒനപഹൠൡกฮะะาำเๅກຂຄຄງຈຊຊຍຍດທນຟມຣລລວວສຫອຮະະາຳຽຽເໄཀཇཉཀྵႠჅაჶᄀᄀᄂᄃᄅᄇᄉᄉᄋᄌᄎᄒᄼᄼᄾᄾᅀᅀᅌᅌᅎᅎᅐᅐᅔᅕᅙᅙᅟᅡᅣᅣᅥᅥᅧᅧᅩᅩᅭᅮᅲᅳᅵᅵᆞᆞᆨᆨᆫᆫᆮᆯᆷᆸᆺᆺᆼᇂᇫᇫᇰᇰᇹᇹḀẛẠỹἀἕἘἝἠὅὈὍὐὗὙὙὛὛὝὝὟώᾀᾴᾶᾼιιῂῄῆῌῐΐῖΊῠῬῲῴῶῼΩΩKÅ℮℮ↀↂ〇〇〡〩ぁゔァヺㄅㄬ一龥가힣ｦﾟ");
      setupRange(localRangeToken, LETTERS_INT);
  ...
  static void setupRange(Token paramToken, int[] paramArrayOfInt)
  {
    ...
    for (int j = 0; j < i; j += 2) {
      paramToken.addRange(paramArrayOfInt[j], paramArrayOfInt[(j + 1)]);
    }
  }
}


/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\org\apache\xerces\internal\impl\xpath\regex\ParserForXMLSchema.class
