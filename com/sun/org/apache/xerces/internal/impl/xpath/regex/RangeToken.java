  ...
  implements Serializable
{
  ...
  protected void compactRanges()
  {
    ...
    while (k < ranges.length)
    {
      ...
      int m = ranges[(j + 1)];
      while ((k < ranges.length) && (m + 1 >= ranges[k])) {
        if (m + 1 == ranges[k])
        {
          if (i != 0) {
  ...
  protected void subtractRanges(Token paramToken)
  {
    ...
    int j = 0;
    int k = 0;
    while ((j < ranges.length) && (k < ranges.length))
    {
      int m = ranges[j];
      int n = ranges[(j + 1)];
      int i1 = ranges[k];
  ...
  synchronized RangeToken getCaseInsensitiveToken()
  {
    ...
    for (int j = 0; j < ranges.length; j += 2) {
      for (k = ranges[j]; k <= ranges[(j + 1)]; k++) {
        if (k > 65535)
        {
          localRangeToken2.addRange(k, k);
        }
        else
        {
          int m = Character.toUpperCase((char)k);
  ...
  private void createMap()
  {
    ...
    for (k = 0; k < ranges.length; k += 2)
    {
      ...
      if (m < 256)
      {
        for (int i1 = m; (i1 <= n) && (i1 < 256); i1++) {
          arrayOfInt[(i1 / 32)] |= 1 << (i1 & 0x1F);
        }
      }
      else
      {
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\org\apache\xerces\internal\impl\xpath\regex\RangeToken.class
