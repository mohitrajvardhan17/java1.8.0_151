  ...
  implements XSModel, XSNamespaceItemList
{
  ...
  public XSModelImpl(SchemaGrammar[] paramArrayOfSchemaGrammar, short paramShort)
  {
    ...
    for (int m = 0; m < i; m++)
    {
      ...
      localObject4 = localSchemaGrammar.getTargetNamespace();
      localObject1[m] = localObject4;
      localObject2[m] = localSchemaGrammar;
      if (localObject4 == SchemaSymbols.URI_SCHEMAFORSCHEMA) {
        k = 1;
      }
    }
    if (k == 0)
    ...
    for (int n = 0; n < i; n++)
    {
      ...
      localObject4 = ((SchemaGrammar)localObject3).getImportedGrammars();
      for (int i1 = localObject4 == null ? -1 : ((Vector)localObject4).size() - 1; i1 >= 0; i1--)
      {
        localSchemaGrammar = (SchemaGrammar)((Vector)localObject4).elementAt(i1);
        for (int i2 = 0; (i2 < i) && (localSchemaGrammar != localObject2[i2]); i2++) {}
        if (i2 == i)
        {
          ...
          localObject1[i] = localSchemaGrammar.getTargetNamespace();
          localObject2[i] = localSchemaGrammar;
          i++;
        }
      }
    }
  ...
  public synchronized XSNamedMap getComponentsByNamespace(short paramShort, String paramString)
  {
    ...
    int i = 0;
    if (paramString != null) {
      while ((i < fGrammarCount) && (!paramString.equals(fNamespaces[i]))) {
        i++;
      }
    }
    while ((i < fGrammarCount) && (fNamespaces[i] != null)) {
      i++;
    }
    if (i == fGrammarCount) {
      return XSNamedMapImpl.EMPTY_MAP;
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\org\apache\xerces\internal\impl\xs\XSModelImpl.class
