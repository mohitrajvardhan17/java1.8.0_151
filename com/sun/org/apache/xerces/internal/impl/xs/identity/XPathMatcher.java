        ...
        else
        {
          ...
          else
          {
            ...
            if (((fCurrentStep[i] == j) || (fCurrentStep[i] > k)) && (fCurrentStep[i]].axis.type == 1))
            {
              ...
              if ((type == 1) && (!name.equals(paramQName)))
              {
                if (fCurrentStep[i] > k) {
                  fCurrentStep[i] = k;
                } else {
  ...
  public void endElement(QName paramQName, XSTypeDefinition paramXSTypeDefinition, boolean paramBoolean, Object paramObject, short paramShort, ShortList paramShortList)
  {
    for (int i = 0; i < fLocationPaths.length; i++)
    {
      ...
      else
      {
        ...
        if ((j >= i) && (fMatched[j] != 0) && ((fMatched[j] & 0x3) != 3))
        {
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\org\apache\xerces\internal\impl\xs\identity\XPathMatcher.class
