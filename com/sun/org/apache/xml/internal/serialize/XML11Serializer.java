  ...
  extends XMLSerializer
{
    ...
    throws IOException
  {
    ...
    for (int j = 0; j < i; j++)
    {
      ...
      if (!XML11Char.isXML11Valid(k))
      {
        j++;
        if (j < i) {
          surrogates(k, paramString.charAt(j));
        } else {
  ...
  protected final void surrogates(int paramInt1, int paramInt2)
    throws IOException
  {
    ...
    if (paramBoolean1) {
      for (i = 0; i < j; i++)
      {
        ...
        if (!XML11Char.isXML11Valid(c))
        {
          i++;
          if (i < j) {
            surrogates(c, paramString.charAt(i));
          } else {
            fatalError("The character '" + c + "' is an invalid XML character");
          }
        }
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\org\apache\xml\internal\serialize\XML11Serializer.class
