...
public class FunctionTable
{
  ...
  String getFunctionName(int paramInt)
  {
    if (paramInt < 37) {
      return m_functions[paramInt].getName();
    }
    return m_functions_customer[(paramInt - 37)].getName();
  }
  
  Function getFunction(int paramInt)
    throws TransformerException
  {
    try
    {
      if (paramInt < 37) {
        return (Function)m_functions[paramInt].newInstance();
      }
      return (Function)m_functions_customer[(paramInt - 37)].newInstance();
    }
    catch (IllegalAccessException localIllegalAccessException)
    {
      throw new TransformerException(localIllegalAccessException.getMessage());
    }
    catch (InstantiationException localInstantiationException)
    {
      throw new TransformerException(localInstantiationException.getMessage());
    }
  }
  
  Object getFunctionID(String paramString)
  {
    Object localObject = m_functionID_customer.get(paramString);
    if (null == localObject) {
      localObject = m_functionID.get(paramString);
    }
    return localObject;
  }
  ...
  {
    Object localObject = getFunctionID(paramString);
    int i;
    if (null != localObject)
    {
      ...
      if (i < 37)
      {
        ...
        m_functionID_customer.put(paramString, new Integer(i));
      }
      m_functions_customer[(i - 37)] = paramClass;
    }
    else
    {
      i = m_funcNextFreeIndex++;
  ...
  {
    Object localObject = m_functionID.get(paramString);
    if (null != localObject) {
      return true;
    }
    localObject = m_functionID_customer.get(paramString);
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\org\apache\xpath\internal\compiler\FunctionTable.class
