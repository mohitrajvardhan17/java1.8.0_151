...
public class Label
{
  ...
  void addToSubroutine(long paramLong, int paramInt)
  {
    ...
    {
      status |= 0x400;
      srcAndRefPositions = new int[(paramInt - 1) / 32 + 1];
    }
  ...
  void visitSubroutine(Label paramLabel, long paramLong, int paramInt)
  {
    if (paramLabel != null)
    {
      ...
      status |= 0x400;
      if (((status & 0x100) != 0) && (!inSameSubroutine(paramLabel)))
      {
        localEdge = new Edge();
        info = inputStackTop;
    ...
    for (Edge localEdge = successors; localEdge != null; localEdge = next) {
      if (((status & 0x80) == 0) || (localEdge != successors.next)) {
        successor.visitSubroutine(paramLabel, paramLong, paramInt);
      }
    }
  }
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\com\sun\xml\internal\ws\org\objectweb\asm\Label.class
