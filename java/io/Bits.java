...
class Bits
{
  ...
  {
    return (char)((paramArrayOfByte[(paramInt + 1)] & 0xFF) + (paramArrayOfByte[paramInt] << 8));
  }
  
  static short getShort(byte[] paramArrayOfByte, int paramInt)
  {
    return (short)((paramArrayOfByte[(paramInt + 1)] & 0xFF) + (paramArrayOfByte[paramInt] << 8));
  }
  
  static int getInt(byte[] paramArrayOfByte, int paramInt)
  {
    return (paramArrayOfByte[(paramInt + 3)] & 0xFF) + ((paramArrayOfByte[(paramInt + 2)] & 0xFF) << 8) + ((paramArrayOfByte[(paramInt + 1)] & 0xFF) << 16) + (paramArrayOfByte[paramInt] << 24);
  }
  
  static float getFloat(byte[] paramArrayOfByte, int paramInt)
  {
    return Float.intBitsToFloat(getInt(paramArrayOfByte, paramInt));
  }
  
  static long getLong(byte[] paramArrayOfByte, int paramInt)
  {
    return (paramArrayOfByte[(paramInt + 7)] & 0xFF) + ((paramArrayOfByte[(paramInt + 6)] & 0xFF) << 8) + ((paramArrayOfByte[(paramInt + 5)] & 0xFF) << 16) + ((paramArrayOfByte[(paramInt + 4)] & 0xFF) << 24) + ((paramArrayOfByte[(paramInt + 3)] & 0xFF) << 32) + ((paramArrayOfByte[(paramInt + 2)] & 0xFF) << 40) + ((paramArrayOfByte[(paramInt + 1)] & 0xFF) << 48) + (paramArrayOfByte[paramInt] << 56);
  }
  ...
  {
    putInt(paramArrayOfByte, paramInt, Float.floatToIntBits(paramFloat));
  }
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\java\io\Bits.class
