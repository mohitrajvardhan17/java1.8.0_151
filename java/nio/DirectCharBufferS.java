  ...
  implements DirectBuffer
{
  ...
  public CharBuffer slice()
  {
    ...
    int m = i << 1;
    assert (m >= 0);
    return new DirectCharBufferS(this, -1, 0, k, k, m);
  }
  ...
  {
    return Bits.swap(unsafe.getChar(ix(checkIndex(paramInt))));
  }
  
  char getUnchecked(int paramInt)
  {
    return Bits.swap(unsafe.getChar(ix(paramInt)));
  }
  ...
  {
    unsafe.putChar(ix(nextPutIndex()), Bits.swap(paramChar));
    return this;
  }
  
  public CharBuffer put(int paramInt, char paramChar)
  {
    unsafe.putChar(ix(checkIndex(paramInt)), Bits.swap(paramChar));
    return this;
  }
  ...
  public CharBuffer put(CharBuffer paramCharBuffer)
  {
    int j;
    int k;
    if ((paramCharBuffer instanceof DirectCharBufferS))
    {
      if (paramCharBuffer == this) {
        throw new IllegalArgumentException();
      }
      DirectCharBufferS localDirectCharBufferS = (DirectCharBufferS)paramCharBuffer;
      j = localDirectCharBufferS.position();
      k = localDirectCharBufferS.limit();
      assert (j <= k);
      int m = j <= k ? k - j : 0;
      int n = position();
      int i1 = limit();
      ...
      unsafe.copyMemory(localDirectCharBufferS.ix(j), ix(n), m << 1);
      localDirectCharBufferS.position(j + m);
      position(n + m);
    }
    else if (hb != null)
    {
  ...
  public CharBuffer subSequence(int paramInt1, int paramInt2)
  {
    ...
    return new DirectCharBufferS(this, -1, i + paramInt1, i + paramInt2, capacity(), offset);
  }
