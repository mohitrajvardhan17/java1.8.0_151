  ...
  implements DirectBuffer
{
  ...
  {
    return Float.intBitsToFloat(Bits.swap(unsafe.getInt(ix(checkIndex(paramInt)))));
  }
  ...
  public FloatBuffer put(FloatBuffer paramFloatBuffer)
  {
    ...
    if ((paramFloatBuffer instanceof DirectFloatBufferS))
    {
      ...
      DirectFloatBufferS localDirectFloatBufferS = (DirectFloatBufferS)paramFloatBuffer;
      j = localDirectFloatBufferS.position();
      k = localDirectFloatBufferS.limit();
      assert (j <= k);
      int m = j <= k ? k - j : 0;
      int n = position();
      int i1 = limit();
      ...
      unsafe.copyMemory(localDirectFloatBufferS.ix(j), ix(n), m << 2);
      localDirectFloatBufferS.position(j + m);
      position(n + m);
    }
    else if (hb != null)
    {
