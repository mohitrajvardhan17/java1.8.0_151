  ...
  extends ClassVisitor
{
  ...
  public static void verify(ClassReader paramClassReader, ClassLoader paramClassLoader, boolean paramBoolean, PrintWriter paramPrintWriter)
  {
    ...
    Type localType = superName == null ? null : Type.getObjectType(superName);
    List localList = methods;
    ArrayList localArrayList = new ArrayList();
    Iterator localIterator = interfaces.iterator();
    ...
    for (int i = 0; i < localList.size(); i++)
    {
      ...
      SimpleVerifier localSimpleVerifier = new SimpleVerifier(Type.getObjectType(name), localType, localArrayList, (access & 0x200) != 0);
      ...
      try
      {
        localAnalyzer.analyze(name, localMethodNode);
        if (!paramBoolean) {
          continue;
        }
      }
  ...
  static void printAnalyzerResult(MethodNode paramMethodNode, Analyzer<BasicValue> paramAnalyzer, PrintWriter paramPrintWriter)
  {
    ...
    paramPrintWriter.println(name + desc);
    for (int i = 0; i < instructions.size(); i++)
    {
      instructions.get(i).accept(localTraceMethodVisitor);
      ...
      else
      {
        for (int j = 0; j < localFrame.getLocals(); j++) {
          localStringBuilder.append(getShortName(((BasicValue)localFrame.getLocal(j)).toString())).append(' ');
        }
        localStringBuilder.append(" : ");
        for (j = 0; j < localFrame.getStackSize(); j++) {
          localStringBuilder.append(getShortName(((BasicValue)localFrame.getStack(j)).toString())).append(' ');
        }
      }
      while (localStringBuilder.length() < maxStack + maxLocals + 1) {
        localStringBuilder.append(' ');
      }
      paramPrintWriter.print(Integer.toString(i + 100000).substring(1));
      paramPrintWriter.print(" " + localStringBuilder + " : " + text.get(text.size() - 1));
    }
    for (i = 0; i < tryCatchBlocks.size(); i++)
    {
      ((TryCatchBlockNode)tryCatchBlocks.get(i)).accept(localTraceMethodVisitor);
      paramPrintWriter.print(" " + text.get(text.size() - 1));
    }
  ...
  public FieldVisitor visitField(int paramInt, String paramString1, String paramString2, String paramString3, Object paramObject)
  {
    ...
    CheckMethodAdapter.checkUnqualifiedName(version, paramString1, "field name");
  ...
  public MethodVisitor visitMethod(int paramInt, String paramString1, String paramString2, String paramString3, String[] paramArrayOfString)
  {
    ...
    if ((!"<init>".equals(paramString1)) && (!"<clinit>".equals(paramString1))) {
      CheckMethodAdapter.checkMethodIdentifier(version, paramString1, "method name");
    }
    ...
    if (checkDataFlow) {
      localCheckMethodAdapter = new CheckMethodAdapter(paramInt, paramString1, paramString2, super.visitMethod(paramInt, paramString1, paramString2, paramString3, paramArrayOfString), labels);
    } else {
      localCheckMethodAdapter = new CheckMethodAdapter(super.visitMethod(paramInt, paramString1, paramString2, paramString3, paramArrayOfString), labels);
    }
    version = version;
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\jdk\internal\org\objectweb\asm\util\CheckClassAdapter.class
