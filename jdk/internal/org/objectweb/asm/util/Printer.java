...
public abstract class Printer
{
  ...
  {
    printList(paramPrintWriter, text);
  }
  ...
  static void printList(PrintWriter paramPrintWriter, List<?> paramList)
  {
    for (int i = 0; i < paramList.size(); i++)
    {
      ...
      if ((localObject instanceof List)) {
        printList(paramPrintWriter, (List)localObject);
      } else {
  ...
  static
  {
    ...
    while ((k = str.indexOf(',', j)) > 0)
    {
      HANDLE_TAG[(i++)] = str.substring(j, k);
      j = k + 1;
    }
  }
}
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\jdk\internal\org\objectweb\asm\util\Printer.class
