  ...
  extends KeyInfoFactory
{
  ...
  public KeyValue newKeyValue(PublicKey paramPublicKey)
    throws KeyException
  {
  ...
  public KeyInfo unmarshalKeyInfo(XMLStructure paramXMLStructure)
    throws MarshalException
  {
    ...
    Node localNode = ((DOMStructure)paramXMLStructure).getNode();
    localNode.normalize();
    Element localElement = null;
    if (localNode.getNodeType() == 9) {
      localElement = ((Document)localNode).getDocumentElement();
    } else if (localNode.getNodeType() == 1) {
    ...
    if (str.equals("KeyInfo")) {
      return new DOMKeyInfo(localElement, new UnmarshalContext(), getProvider());
    }
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\org\jcp\xml\dsig\internal\dom\DOMKeyInfoFactory.class
