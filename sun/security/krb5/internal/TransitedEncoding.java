...
public class TransitedEncoding
{
    ...
    throws Asn1Exception, IOException
  {
    ...
    if ((localDerValue.getTag() & 0x1F) == 0) {
      trType = localDerValue.getData().getBigInteger().intValue();
    } else {
      throw new Asn1Exception(906);
    }
    localDerValue = paramDerValue.getData().getDerValue();
    if ((localDerValue.getTag() & 0x1F) == 1) {
      contents = localDerValue.getData().getOctetString();
    } else {
      throw new Asn1Exception(906);
    }
...
/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\sun\security\krb5\internal\TransitedEncoding.class
