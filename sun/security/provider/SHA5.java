  ...
  extends DigestBase
{
  ...
  SHA5(String paramString, int paramInt, long[] paramArrayOfLong)
  {
    ...
    initialHashes = paramArrayOfLong;
    state = new long[8];
    W = new long[80];
    implReset();
  }
  ...
  final void implDigest(byte[] paramArrayOfByte, int paramInt)
  {
    ...
    int i = (int)bytesProcessed & 0x7F;
    int j = i < 112 ? 112 - i : 240 - i;
    engineUpdate(padding, 0, j + 8);
    ByteArrayAccess.i2bBig4((int)(l >>> 32), buffer, 120);
    ByteArrayAccess.i2bBig4((int)l, buffer, 124);
    implCompress(buffer, 0);
    ByteArrayAccess.l2bBig(state, 0, paramArrayOfByte, paramInt, engineGetDigestLength());
  }
  
  private static long lf_ch(long paramLong1, long paramLong2, long paramLong3)
  {
    return paramLong1 & paramLong2 ^ (paramLong1 ^ 0xFFFFFFFFFFFFFFFF) & paramLong3;
  }
  ...
  {
    return lf_S(paramLong, 28) ^ lf_S(paramLong, 34) ^ lf_S(paramLong, 39);
  }
  
  private static long lf_sigma1(long paramLong)
  {
    return lf_S(paramLong, 14) ^ lf_S(paramLong, 18) ^ lf_S(paramLong, 41);
  }
  
  private static long lf_delta0(long paramLong)
  {
    return lf_S(paramLong, 1) ^ lf_S(paramLong, 8) ^ lf_R(paramLong, 7);
  }
  
  private static long lf_delta1(long paramLong)
  {
    return lf_S(paramLong, 19) ^ lf_S(paramLong, 61) ^ lf_R(paramLong, 6);
  }
  
  final void implCompress(byte[] paramArrayOfByte, int paramInt)
  {
    ByteArrayAccess.b2lBig128(paramArrayOfByte, paramInt, W);
    for (int i = 16; i < 80; i++) {
    ...
    for (int j = 0; j < 80; j++)
    {
      long l9 = l8 + lf_sigma1(l5) + lf_ch(l5, l6, l7) + ROUND_CONSTS[j] + W[j];
  ...
  {
    SHA5 localSHA5 = (SHA5)super.clone();
    state = ((long[])state.clone());
    W = new long[80];
    return localSHA5;
  }
    ...
    extends SHA5
  {
    ...
    public SHA512()
    {
      super(64, INITIAL_HASHES);
    }
  }
}


/* Location:              C:\Program Files (x86)\Java\jre1.8.0_151\lib\rt.jar!\sun\security\provider\SHA5.class
